...
impl Plane {
    ...
    pub fn distance(&self, to: Vec3<f64>) -> f64 { self.normal.dot(to) - self.d }

    // fn center(&self) -> Vec3<f32> { *self.normal * self.d }

    pub fn projection(&self, v: Vec3<f64>) -> Vec3<f64> { v - *self.normal * self.distance(v) }
...
impl Projection<Plane> for Vec3<f64> {
